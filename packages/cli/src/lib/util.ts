...
const getAllFiles = function (root, ignoreDir, fileExt, options) {
  ...
  options.depth -= 1;
  var res = [],
    files = options && options.cwd ? fs.readdirSync(path.join(options.cwd, root)) : fs.readdirSync(root);
  files.forEach(function (file) {
    var pathname = root + '/' + file,
      stat = fs.lstatSync(options && options.cwd ? path.join(options.cwd, pathname) : pathname);

    if ((typeof ignoreDir === 'string' && file === ignoreDir) || (Array.isArray(ignoreDir) && ignoreDir.indexOf(file) !== -1)) {
      return;
    }
    const ext = path.parse(pathname).ext;
    if (fileExt && !stat.isDirectory() && (fileExt === ext || (fileExt instanceof RegExp && fileExt.test(ext)))) {
      res.push({
        path: pathname,
        dir: false,
      });
...
const getTemplateJs = function (proxy, YaConfig) {
  ...
  loaderTemplate.some((item) => {
    if (YaConfig.templates[item] && YaConfig.templates[item].js) {
      result = YaConfig.templates[item].js;
    }
  });
...
const getTemplateCss = function (proxy, YaConfig) {
  let fileMap = proxy;
  if (fileMap.css) {
    return fileMap.css;
  }
  ...
  loaderTemplate.some((item) => {
    if (YaConfig.templates[item] && YaConfig.templates[item].css) {
      result = YaConfig.templates[item].css;
    }
  });
...
const handleHtml = function (result, gamesite, proxy) {
  ...
  if (proxy.jsRequires && proxy.jsRequires.length > 0) {
    const jsRequires = proxy.jsRequires;
    for (let i = 0; i < jsRequires.length; i++) {
      insertScript(window.document, $(jsLoader), '/' + path.join(gamesite, jsRequires[i]), $);
    }
  }
  ...
  if (!YaConfig.cssInjector) {
    YaConfig.cssInjector = [];
  }
  ...
  const jsLoaderPath = getTemplateJs(proxy, YaConfig);
  const cssLoaderPath = getTemplateCss(proxy, YaConfig);

  jsLoaderPath && jsLoader && (jsLoader.src = '/' + path.join(gamesite, jsLoaderPath));
  cssLoaderPath && cssLoader && (cssLoader.href = '/' + path.join(gamesite, cssLoaderPath));

  /*注入新的css*/

  if (proxy.cssRequires && proxy.cssRequires.length > 0) {
    const cssRequires = proxy.cssRequires;
    for (const element of cssRequires) {
      insertLinkCss(window.document, $(cssLoader), '/' + path.join(gamesite, element));
    }
  }
...
function printServerUrls(urls: ResolvedServerUrls, optionsHost: string | boolean | undefined): void {
  ...
  if (urls.network.length === 0 && optionsHost === undefined) {
    console.log(colors.dim(`  ${colors.green('➜')}  ${colors.bold('Network')}: use `) + colors.bold('--host') + colors.dim(' to expose'));
  }
}
...
async function getLocalhostAddressIfDiffersFromDNS(): Promise<string | undefined> {
  const [nodeResult, dnsResult] = await Promise.all([dns.lookup('localhost'), dns.lookup('localhost', { verbatim: true })]);
...
async function resolveHostname(optionsHost: string | boolean | undefined): Promise<Hostname> {
...
async function resolveServerUrls(server: Server, options: CommonServerOptions, config: any = { rawBase: '' }): Promise<ResolvedServerUrls> {
...
export { getAllFiles, insertLinkCss, insertScript, handleHtml, getDefaultHosts, printServerUrls, resolveServerUrls };
