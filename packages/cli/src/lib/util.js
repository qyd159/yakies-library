...
const getAllFiles = function (root, ignoreDir, fileExt, options) {
    ...
    options.depth -= 1;
    var res = [], files = options && options.cwd ? fs.readdirSync(path.join(options.cwd, root)) : fs.readdirSync(root);
    files.forEach(function (file) {
        var pathname = root + '/' + file, stat = fs.lstatSync(options && options.cwd ? path.join(options.cwd, pathname) : pathname);
...
async function resolveServerUrls(server, options, config = { rawBase: '' }) {
