...
export default function (argv) {
    ...
    const os = require('os'), portfinder = require('portfinder'), opener = require('opener'), fs = require('fs'), url = require('url');
    ...
    if (argv.h || argv.help) {
        console.log([
            ...
            '',
            '  -S --tls   Enable secure request serving with TLS/SSL (HTTPS)',
    ...
    var port = argv.p || argv.port || parseInt(process.env.PORT, 10), host = argv.a || '0.0.0.0', tls = argv.S || argv.tls, sslPassphrase = process.env.NODE_HTTP_SERVER_SSL_PASSPHRASE, proxy = argv.P || argv.proxy, proxyOptions = argv['proxy-options'], utc = argv.U || argv.utc, version = argv.v || argv.version, logger;
        ...
        logger = {
            ...
            request: function (req, res, error) {
                ...
                var ip = argv['log-ip']
                    ...
                    : '';
                if (error) {
                    logger.info('[%s] %s "%s %s" Error (%s): "%s"', date, ip, chalk.red(req.method), chalk.red(req.url), chalk.red(error.status.toString()), chalk.red(error.message));
                }
                else {
                    logger.info('[%s] %s "%s %s" "%s"', date, ip, chalk.cyan(req.method), chalk.cyan(req.url), req.headers['user-agent']);
                }
            }
        };
    }
    ...
    function listen(port) {
        ...
        server.listen(port, host, function () {
            ...
            logger.info([chalk.yellow('\nhttp-server version: '), chalk.cyan(appVersion)].join(''));
            logger.info([
