...
<script lang="js">
  ...
  computed: {
    ...
    vdrStick() {
      return (stick) => {
        const stickStyle = {
          width: stick[1] === 'm'?'100%':`${this.stickSize / this.parentScaleX}px`,
          height: stick[0] === 'm' ? '100%' : `${this.stickSize / this.parentScaleY}px`,
          zIndex: stick.indexOf('m')!==-1?1:999
        };
  ...
  methods: {
    ...
    calcResizeLimits() {
      ...
      let { maxh: maxHeight, maxw: maxWidth } = this;
      ...
      if(maxHeight) {
        limits.top.min = Math.max(parentLim, top - (maxHeight - height));
        limits.bottom.min = Math.max(parentLim, bottom - (maxHeight - height));
      }

      if(maxWidth) {
        limits.left.min = Math.max(parentLim, left - (maxWidth - width));
        limits.right.min = Math.max(parentLim, right - (maxWidth - width));
      }
